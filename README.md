...
| `history` / `mem` | Print all turns in the current conversation thread |
...
+-- src/
    ...
    ¦   +-- climate_optimization_node.py # Plan+Replan climate pipeline
    ¦   +-- information_node.py          # show_guide, handle_unknown
    ¦   +-- planning_node.py             # Shared planning utilities
...
| `climate_optimization` | Comply with code, then optimise orientation for local climate |
...
classify_input
  ¦
  +-- design_building / climate_optimization --? building_planner
  ¦                                                    ¦
  ¦                                        +--------------------------------+
  ¦                                        ¦                                ¦
  ...
  ¦                                                    climate_replan
  ¦                                                               ¦
  ¦                                                    execute_climate_step (loop)
  ¦                                                               ¦
  ¦                                                    climate_summary ? done
  ¦
  +-- use_tool ------------? execute_gh_tool ? done
  +-- show_guide ----------? show_guide ? done
  +-- general_question ----? determine_search_need ? [perform_web_search ?] answer ? done
...
| `src/graphs/main_graph.py` | Assembles all nodes and edges; call `build_main_graph()` |
...
| `src/nodes/climate_optimization_node.py` | Climate web-search ? Plan+Replan orientation pipeline |
...
| `src/notebooks/agent_system_tests.ipynb` | End-to-end test notebook (all routing branches, classifier validation, graph visualisation) |
